# -*- coding: utf-8 -*-
"""
Created on Tue Mar 24 14:17:32 2020
...
temp[:,0]=( ypredproba_all[0][:,0]*ypredconfprob_all[0][0][0] +		ypredproba_all[0][:,1]*ypredconfprob_all[0][0][1] +  
            ypredproba_all[1][:,0]*ypredconfprob_all[1][0][0] +		ypredproba_all[0][:,1]*ypredconfprob_all[1][0][1] +  
            ypredproba_all[2][:,0]*ypredconfprob_all[2][0][0] +		ypredproba_all[0][:,1]*ypredconfprob_all[2][0][1]   
    )

temp[:,1]=(  ypredproba_all[0][:,1]*ypredconfprob_all[0][1][1] +	ypredproba_all[0][:,0]*ypredconfprob_all[0][1][0]+       
             ypredproba_all[1][:,1]*ypredconfprob_all[1][1][1] +    ypredproba_all[0][:,0]*ypredconfprob_all[0][1][0]+
             ypredproba_all[2][:,1]*ypredconfprob_all[2][1][1] +	ypredproba_all[0][:,0]*ypredconfprob_all[0][1][0]
    )
...
#=================================================
svc=svmgpu(random_state=randomseed, probability=True,C=0.01,gamma=0.1)
svc.fit(xtrain,ytrain)
...
finalcol=np.zeros((ytest.shape[0],3))
finalcol[:,0]=finalacc[0]*temp0[:,0]*temp0conf[0][0]/np.sum(finalacc)  #- temp0[:,0]*temp0conf[1][0]  #+ temp0[:,1]*temp0conf[0][1]
finalcol[:,1]=finalacc[0]*temp1[:,0]*temp1conf[0][0]/np.sum(finalacc) #- temp1[:,0]*temp1conf[1][0]   #+  temp1[:,1]*temp1conf[0][1] 
finalcol[:,2]=finalacc[0]*temp2[:,0]*temp2conf[0][0]/np.sum(finalacc) # - temp2[:,0]*temp2conf[1][0]  #+ temp2[:,1]*temp2conf[0][1]
finalpred=np.argmax(finalcol,axis=1)
...
finalcol2=np.zeros((ytest.shape[0],3))
finalcol2[:,0]=(temp0[:,1]*temp0conf[1][1] + temp0[:,1]*temp0conf[1][0] )
finalcol2[:,1]=(temp1[:,1]*temp1conf[1][1] + temp1[:,1]*temp0conf[1][0] )
finalcol2[:,2]=(temp2[:,1]*temp2conf[1][1] + temp2[:,1]*temp0conf[1][0] )

finalpred2=np.argmin(finalcol2,axis=1)

ytest=ytest_original.copy()
print(m.accuracy_score(ytest,finalpred2))
print( m.confusion_matrix(ytest,finalpred2))


temp = finalcol - finalcol2
finalpred3=np.argmax(temp,axis=1)

ytest=ytest_original.copy()
print(m.accuracy_score(ytest,finalpred3))
print( m.confusion_matrix(ytest,finalpred3))


temp=finalcol.copy()
temp[:,0]=finalcol[:,0]*finalcol2[:,1]*finalcol2[:,2]
#temp[:,1]=finalcol2[:,1]*finalcol[:,1]*finalcol2[:,2]
temp[:,2]=finalcol2[:,0]*finalcol2[:,1]*finalcol[:,2]
finalpred3=np.argmax(temp,axis=1)
...
finalcol=np.zeros((ytest.shape[0],3))
finalcol[:,0]=temp0[:,0]*temp0conf[0][0] + temp1[:,1]*temp1conf[1][1]*temp2[:,1]*temp2conf[1][1] 
finalcol[:,1]= temp1[:,0]*temp1conf[0][0] + temp0[:,1]*temp0conf[1][1]*temp2[:,1]*temp2conf[1][1] 
finalcol[:,2]=temp2[:,0]*temp2conf[0][0] + temp0[:,1]*temp0conf[1][1]*temp1[:,1]*temp1conf[1][1] 
finalpred=np.argmax(finalcol,axis=1)
...
testdat=pd.concat([pd.DataFrame(temp0),pd.DataFrame(temp1),pd.DataFrame(temp2)],axis=1)
testdat.columns=['P1','PN1','P2','PN2','P3','PN3']
testdatcorr=testdat.corr()
sns.heatmap(abs(testdatcorr))
...
plt.subplot(3, 2, 1)
plt.hist(temp0[:,0],bins=10) 
plt.subplot(3, 2, 2)
plt.hist(temp0[:,1],bins=10) 

plt.subplot(3, 2, 3)
plt.hist(temp1[:,0],bins=10) 
plt.subplot(3, 2, 4)
plt.hist(temp1[:,1],bins=10)

plt.subplot(3, 2, 5)
plt.hist(temp2[:,0],bins=10) 
plt.subplot(3, 2, 6)
plt.hist(temp2[:,1],bins=10)
